...
allExtWithSvg = get_available_image_extensions()
allExtWithSvg.append('svg')
allExtWithSvgValidator = FileExtensionValidator(
    allowed_extensions=allExtWithSvg,
)

class SVGAndImageFormField(DjangoImageField):
    default_validators = [allExtWithSvgValidator, ]
...
#         try:
        ...
        # content = ifile.getvalue().decode('UTF-8')
        if self.is_svg(ifile):
            test_file.image = ifile
            test_file.content_type = 'image/svg+xml'
        else:
